...
class PluginsViewModelTest {
    ...
    @Test
    @DisplayName("plugins tab shows only installed plugins")
    fun `plugins tab shows only installed plugins`() =
        runTest {
            val filtered = filterPlugins(samplePlugins, TAB_PLUGINS, "")
            assertEquals(1, filtered.size)
            assertTrue(filtered.all { it.isInstalled })
        }

    @Test
    @DisplayName("non-plugins tab returns empty list")
    fun `non-plugins tab returns empty list`() =
        runTest {
            val appsFiltered = filterPlugins(samplePlugins, TAB_APPS, "")
            assertTrue(appsFiltered.isEmpty())
            val skillsFiltered = filterPlugins(samplePlugins, TAB_SKILLS, "")
            assertTrue(skillsFiltered.isEmpty())
        }
    ...
    fun `search by name filters correctly`() =
        runTest {
            val filtered = filterPlugins(samplePlugins, TAB_PLUGINS, "HTTP")
            assertEquals(1, filtered.size)
    ...
    fun `search by description filters correctly`() =
        runTest {
            val filtered = filterPlugins(samplePlugins, TAB_PLUGINS, "REST")
            assertEquals(1, filtered.size)
            assertEquals("HTTP Channel", filtered.first().name)
        }
    ...
    fun `no match returns empty list`() =
        runTest {
            val filtered = filterPlugins(samplePlugins, TAB_PLUGINS, "nonexistent")
            assertTrue(filtered.isEmpty())
        }

    @Test
    @DisplayName("install moves plugin to plugins tab")
    fun `install moves plugin to plugins tab`() =
        runTest {
    ...
    @Test
    @DisplayName("default selected tab is apps")
    fun `default selected tab is apps`() =
        runTest {
            assertEquals(0, TAB_APPS)
        }
}
...
private fun filterPlugins(
    ...
    val tabFiltered =
        when (tab) {
            TAB_PLUGINS -> plugins.filter { it.isInstalled }
            else -> emptyList()
        }
