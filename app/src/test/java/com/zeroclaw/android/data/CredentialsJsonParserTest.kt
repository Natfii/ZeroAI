...
class CredentialsJsonParserTest {
    ...
    fun `parses valid credentials json with all fields`() {
        val json =
            ...
            {
              "claudeAiOauth": {
                "accessToken": "test_anthropic_oauth_access_not_real",
                "refreshToken": "test_anthropic_oauth_refresh_not_real",
        ...
        assertEquals("test_anthropic_oauth_access_not_real", result.accessToken)
        assertEquals("test_anthropic_oauth_refresh_not_real", result.refreshToken)
    ...
    fun `parses credentials without expiresAt`() {
        val json =
            ...
            {
              "claudeAiOauth": {
                "accessToken": "test_anthropic_oauth_access",
                "refreshToken": "test_anthropic_oauth_refresh"
              }
            }
            """.trimIndent()
        ...
        assertNotNull(result)
        assertEquals("test_anthropic_oauth_access", result!!.accessToken)
        assertEquals("test_anthropic_oauth_refresh", result.refreshToken)
