...
class ProviderRegistryTest {
    ...
    fun `findById resolves aliases`() {
        ...
        assertEquals(ProviderAuthType.API_KEY_ONLY, google.authType)

        val gemini = ProviderRegistry.findById("gemini")
        assertNotNull(gemini)
        assertEquals("google-gemini", gemini!!.id)

        val chatgpt = ProviderRegistry.findById("chatgpt")
        assertNotNull(chatgpt)
        assertEquals("openai", chatgpt!!.id)

        val codex = ProviderRegistry.findById("openai-codex")
        assertNotNull(codex)
        assertEquals("openai", codex!!.id)
    }
    ...
    @Test
    @DisplayName("allByCategory returns non-empty primary group")
    fun `allByCategory returns non-empty primary group`() {
    ...
    @Test
    @DisplayName("all providers have helpText populated")
    fun `all providers have helpText populated`() {
        val priorityIds =
            listOf(
                "openai",
                "anthropic",
    ...
    @Test
    @DisplayName("ollama has no keyPrefix")
    fun `ollama has no keyPrefix`() {
        val provider = ProviderRegistry.findById("ollama")
        assertNotNull(provider, "ollama should exist")
        assertTrue(
            provider!!.keyPrefix.isEmpty(),
            "ollama should have empty keyPrefix",
        )
    }
    ...
    @Test
    @DisplayName("openrouter has API_KEY_ONLY auth type and correct prefix")
    fun `openrouter has API_KEY_ONLY auth type and correct prefix`() {
        val provider = ProviderRegistry.findById("openrouter")
