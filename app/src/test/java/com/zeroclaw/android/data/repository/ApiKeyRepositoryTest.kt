...
class ApiKeyRepositoryTest {
        ...
        runTest {
            ...
            val key =
                ApiKey(
                    id = "1",
                    provider = "Anthropic",
                    key = "test_anthropic_oauth_access",
                    refreshToken = "test_anthropic_oauth_refresh",
                    expiresAt = 1739836800000L,
                )
            repo.save(key)
            val retrieved = repo.getById("1")
            assertEquals("test_anthropic_oauth_refresh", retrieved?.refreshToken)
    ...
    fun `export and import preserves OAuth fields`() =
        runTest {
            ...
            val imported = newRepo.keys.first()
            assertEquals(1, imported.size)
            assertEquals("test_anthropic_oauth_refresh", imported[0].refreshToken)
        ...
        runTest {
            ...
            val key =
                ApiKey(
                    id = "1",
                    provider = "anthropic",
                    key = "test_anthropic_oauth_valid",
                    refreshToken = "test_anthropic_oauth_refresh",
                    expiresAt = farFuture,
                )
            repo.save(key)
        ...
        runTest {
            ...
            val key =
                ApiKey(
                    id = "1",
                    provider = "anthropic",
                    key = "test_anthropic_oauth_expired",
                    refreshToken = "test_refresh_will_fail",
                    expiresAt = expiredAt,
                )
            repo.save(key)
