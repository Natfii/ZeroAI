...
class SkillsBridgeTest {
    ...
    @Test
    @DisplayName("saveCommunitySkill delegates to FFI")
    fun `saveCommunitySkill delegates to FFI`() =
        runTest {
            every {
                com.zeroclaw.ffi.saveCommunitySkill("my-skill", "# Content")
            } returns Unit

            bridge.saveCommunitySkill("my-skill", "# Content")
    ...
    @Test
    @DisplayName("saveCommunitySkill propagates FfiException")
    fun `saveCommunitySkill propagates FfiException`() =
        runTest {
            every {
                com.zeroclaw.ffi.saveCommunitySkill(any(), any())
            } throws FfiException.ConfigException("invalid name")

            assertThrows<FfiException> {
                bridge.saveCommunitySkill("../evil", "x")
            }
        }

    @Test
    @DisplayName("toggleCommunitySkill delegates to FFI")
    fun `toggleCommunitySkill delegates to FFI`() =
        runTest {
            every {
                com.zeroclaw.ffi.toggleCommunitySkill("my-skill", false)
            } returns Unit

            bridge.toggleCommunitySkill("my-skill", false)
    ...
    @Test
    @DisplayName("toggleCommunitySkill propagates FfiException")
    fun `toggleCommunitySkill propagates FfiException`() =
        runTest {
            every {
                com.zeroclaw.ffi.toggleCommunitySkill(any(), any())
            } throws FfiException.InvalidArgument("skill not found")

            assertThrows<FfiException> {
                bridge.toggleCommunitySkill("nonexistent", true)
            }
        }

    @Test
    @DisplayName("getSkillContent delegates to FFI")
    fun `getSkillContent delegates to FFI`() =
        runTest {
            every {
                com.zeroclaw.ffi.getSkillContent("my-skill")
            } returns "---\nname: my-skill\n---\n# Skill"

            val result = bridge.getSkillContent("my-skill")

            assertEquals("---\nname: my-skill\n---\n# Skill", result)
            verify(exactly = 1) { com.zeroclaw.ffi.getSkillContent("my-skill") }
        }

    @Test
    @DisplayName("getSkillContent propagates FfiException")
    fun `getSkillContent propagates FfiException`() =
        runTest {
            every {
                com.zeroclaw.ffi.getSkillContent(any())
            } throws FfiException.InvalidArgument("skill not found")

            assertThrows<FfiException> {
                bridge.getSkillContent("nonexistent")
            }
        }
