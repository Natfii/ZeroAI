 ...
 * @param showProviderDropdown Whether to render the provider dropdown at all.
...
fun ProviderCredentialForm(
    ...
    showProviderDropdown: Boolean = true,
    ...
    val needsUrl =
        when (credentialTypeOverride) {
            SlotCredentialType.URL_KEY -> true
    ...
    val needsKey =
        when (credentialTypeOverride) {
            SlotCredentialType.API_KEY -> true
            SlotCredentialType.URL_KEY -> false
            SlotCredentialType.OAUTH -> false
            null -> authType == ProviderAuthType.API_KEY_ONLY
        }
    val showKeyField =
        when (credentialTypeOverride) {
            SlotCredentialType.API_KEY,
    ...
    Column(modifier = modifier) {
        ...
        val helpText = providerHelpText(providerInfo, credentialTypeOverride)
        ...
        if (!oauthConnected &&
            credentialTypeOverride != SlotCredentialType.OAUTH &&
            providerInfo?.keyCreationUrl?.isNotEmpty() == true
        ) {
            TextButton(
                onClick = {
...
private fun providerHelpText(
    providerInfo: ProviderInfo?,
    credentialTypeOverride: SlotCredentialType?,
): String? {
    if (providerInfo == null) return null
    if (providerInfo.id == "openai" && credentialTypeOverride == SlotCredentialType.OAUTH) {
        return "ChatGPT login uses your ChatGPT access (Plus/Pro/etc. when required). " +
            "API billing only applies when you use a direct OpenAI API key."
    }
    return providerInfo.helpText.takeIf { it.isNotBlank() }
}
