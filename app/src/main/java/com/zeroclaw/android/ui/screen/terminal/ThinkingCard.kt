...
/** Size of the compact mascot shown in the terminal thinking header. */
private const val HEADER_MASCOT_SIZE_DP = 32
        ...
        ElevatedCard(
            ...
            Column(
                ...
                Row(
                    ...
                    MiniZeroMascot(
                        state =
                            deriveMascotState(phase, toolResults),
                        size = HEADER_MASCOT_SIZE_DP.dp,
                        contentDescription = null,
                    )
                    Spacer(modifier = Modifier.width(BODY_SPACING_DP.dp))
...
/**
 * Derives the mascot state from the streaming phase and latest tool result.
 *
 * When a tool just completed, briefly reflects success or error.
 * Otherwise falls back to the phase-based default.
 */
private fun deriveMascotState(
    phase: StreamingPhase,
    toolResults: List<ToolResultEntry>,
): MiniZeroMascotState {
    val lastResult = toolResults.lastOrNull()
    if (lastResult != null && phase == StreamingPhase.TOOL_EXECUTING) {
        return if (lastResult.success) {
    ...
    return when (phase) {
        ...
        StreamingPhase.ERROR -> MiniZeroMascotState.Error
        StreamingPhase.CANCELLED -> MiniZeroMascotState.Angry
        else -> MiniZeroMascotState.Thinking
    }
}
