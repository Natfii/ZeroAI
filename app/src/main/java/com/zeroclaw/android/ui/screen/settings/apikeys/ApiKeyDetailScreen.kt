 ...
 * @param providerHint Optional provider ID to preselect when creating a new key.
...
fun ApiKeyDetailScreen(
    ...
    var providerId by remember(existingKey, providerHint) {
        mutableStateOf(existingKey?.provider ?: providerHint.orEmpty())
    }
            ...
            if (isNewKey) {
                NewKeyForm(
                    ...
                    onOAuthLogin = { context ->
                        apiKeysViewModel.startOAuthLogin(context, providerId)
                    },
                )
            } else {
                EditKeyForm(
