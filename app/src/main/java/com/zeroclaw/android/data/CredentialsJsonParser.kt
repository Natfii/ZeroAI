...
/**
 * Parsed OAuth credential payload from a Claude Code `.credentials.json` file.
 *
 * @property provider Canonical provider identifier for the imported profile.
 * @property accessToken OAuth access token.
 * @property refreshToken OAuth refresh token.
 * @property expiresAt Epoch milliseconds when the access token expires, or `0`
 *   when the source file omits expiry metadata.
 */
data class ImportedOAuthCredentials(
    val provider: String,
...
/**
 * Parses Claude Code `.credentials.json` files into [ImportedOAuthCredentials].
 *
...
object CredentialsJsonParser {
     ...
     * @return Parsed OAuth credentials, or null
     *   if the file does not contain a valid `claudeAiOauth` block.
     */
    @Suppress("TooGenericExceptionCaught")
    fun parse(jsonContent: String): ImportedOAuthCredentials? {
        return try {
