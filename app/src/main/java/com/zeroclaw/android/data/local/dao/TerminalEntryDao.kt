...
interface TerminalEntryDao {
    ...
    /**
     * Deletes entries whose content starts with the given prefix.
     *
     * @param prefix Content prefix to match (uses SQLite LIKE with trailing %).
     */
    @Query("DELETE FROM terminal_entries WHERE content LIKE :prefix || '%'")
    suspend fun deleteByContentPrefix(prefix: String)
}
