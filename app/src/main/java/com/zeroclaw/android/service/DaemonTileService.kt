...
/**
 * Quick Settings tile that toggles the ZeroAI daemon on and off.
 *
 ...
 * [ACTION_START][ZeroAIDaemonService.ACTION_START] or
 * [ACTION_STOP][ZeroAIDaemonService.ACTION_STOP] intent to the foreground
 ...
 * so the Zero glyph is recognisable regardless of the system tint applied
 * by the launcher.
 *
...
class DaemonTileService : TileService() {
     ...
     *   [ZeroAIDaemonService.ACTION_START] to start the daemon.
     * - [ServiceState.RUNNING]: sends
     *   [ZeroAIDaemonService.ACTION_STOP] to stop the daemon.
    ...
    override fun onClick() {
        ...
        when (currentServiceState()) {
            ...
            ServiceState.ERROR,
            -> sendDaemonIntent(ZeroAIDaemonService.ACTION_START)

            ServiceState.RUNNING ->
                sendDaemonIntent(ZeroAIDaemonService.ACTION_STOP)
     ...
     */
    private fun bridgeOrNull(): DaemonServiceBridge? = (applicationContext as? ZeroAIApplication)?.daemonBridge

    /**
     * Sends an intent to [ZeroAIDaemonService] with the specified action.
     *
     ...
     * on Android 8.0+.
     *
     * @param action One of [ZeroAIDaemonService.ACTION_START],
     *   [ZeroAIDaemonService.ACTION_STOP], or
     *   [ZeroAIDaemonService.ACTION_RETRY].
     */
    private fun sendDaemonIntent(action: String) {
        val intent =
            Intent(this, ZeroAIDaemonService::class.java).apply {
                this.action = action
            }
    ...
    companion object {
        private const val LABEL_RUNNING = "ZeroAI: Running"
        private const val LABEL_STOPPED = "ZeroAI: Stopped"
        private const val LABEL_ERROR = "ZeroAI: Error"
        private const val LABEL_TRANSITIONING = "ZeroAI: ..."
    }
}
