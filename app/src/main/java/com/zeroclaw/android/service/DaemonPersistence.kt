 ...
 *
 * Uses the same plain preferences file ([DaemonServicePrefs.PREFS_NAME]) as the
...
class DaemonPersistence(
    ...
    fun recordRunning(
        ...
        check(
            securePrefs
                ...
                .commit(),
        ) { "Encrypted storage unavailable: unable to persist daemon config" }
        check(
            plainPrefs
                .edit()
                ...
                .commit(),
        ) { "Failed to persist daemon runtime state" }
    }
    ...
    fun recordStopped() {
        check(
            plainPrefs
                ...
                .remove(KEY_PORT)
                .commit(),
        ) { "Failed to persist daemon stopped state" }
        check(
            securePrefs
                .edit()
                .remove(KEY_CONFIG_TOML)
                .commit(),
        ) { "Encrypted storage unavailable: unable to clear daemon config" }
    }
     ...
     * e.g. in [ZeroAIDaemonService.handleStickyRestart] before rebuilding
