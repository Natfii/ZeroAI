...
import com.zeroclaw.ffi.unregisterWebRenderer
import java.io.File
 ...
 * [ZeroAIApplication][com.zeroclaw.android.ZeroAIApplication] and
...
class DaemonServiceBridge(
     ...
     *
     * Set after construction from [ZeroAIApplication.onCreate] because the
    ...
    /**
     * Optional [CredentialBridge] for per-provider API key resolution.
     *
     * Set by [com.zeroclaw.android.ZeroAIApplication] after creating
     * the [com.zeroclaw.android.data.repository.ApiKeyRepository].
     * When non-null, registered alongside [EventBridge] at daemon start.
     */
    var credentialBridge: CredentialBridge? = null

    /**
     * Optional [BackgroundWebRenderer] for headless WebView rendering.
     *
     * Initialised via [initWebRenderer] from
     * [ZeroAIDaemonService.onCreate][ZeroAIDaemonService] after this bridge is
     * obtained from [ZeroAIApplication]. When non-null, it is registered with
     * the Rust FFI layer after a successful [start] and unregistered before
     * [stop], following the same lifecycle pattern as [eventBridge].
     */
    private var webRenderer: BackgroundWebRenderer? = null
    ...
    /**
     * Creates the [BackgroundWebRenderer] for headless WebView rendering.
     *
     * Must be called from [ZeroAIDaemonService.onCreate] with the application
     * context so the [WebView][android.webkit.WebView] does not leak an Activity
     * reference. The renderer is registered with the Rust FFI layer during
     * [start] and torn down during [stop].
     *
     * @param context Application context (not an Activity context).
     */
    fun initWebRenderer(context: Context) {
        appContext = context
        webRenderer = BackgroundWebRenderer(context)
    }
     ...
     * [withContext].
     *
     * This is designed to be called once from [ZeroAIApplication.onCreate]
     ...
     *
     * Called by [ZeroAIDaemonService] during startup when stale artifacts are
     ...
     *
     * Called by [ZeroAIDaemonService] when [checkMemoryHealth] returns
    ...
    /**
     * Starts the ZeroAI daemon with the provided configuration.
     *
            ...
            withContext(ioDispatcher) {
                ...
                if (renderer != null) {
                    try {
                        ...
                        // best-effort; may already be unregistered
                    }
                    renderer.shutdown()
                    webRenderer = null
                }
                eventBridge?.unregister()
                credentialBridge?.unregister()
                stopDaemon()
            }
            _lastError.value = null
            _serviceState.value = ServiceState.STOPPED
        ...
        val status =
            try {
                ...
                throw FfiException.StateException(
                    e.message ?: "malformed status JSON from native layer",
                )
            }
    ...
    /**
     * Marks the daemon as failed after a runtime status-poll error.
     *
     * @param detail Sanitized human-readable error detail.
     */
    fun reportRuntimeFailure(detail: String) {
        _lastError.value = detail
        _serviceState.value = ServiceState.ERROR
    }

    private fun applyPolledStatus(status: DaemonStatus) {
        _lastStatus.value = status
        if (status.running) {
            _lastError.value = null
            _serviceState.value = ServiceState.RUNNING
        } else if (_serviceState.value != ServiceState.STOPPING) {
            _restartRequired.value = false
            _serviceState.value = ServiceState.STOPPED
        }
    }
     ...
     *
     * Creates the standard `ZeroAI` workspace structure (5 subdirectories
     ...
     *
     * @param agentName Name for the AI agent (empty defaults to "ZeroAI").
