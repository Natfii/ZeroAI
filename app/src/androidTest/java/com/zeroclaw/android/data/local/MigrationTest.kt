...
class MigrationTest {
    ...
    fun migrate_3_to_4_addsChatMessagesTable() {
        helper.createDatabase(TEST_DB, 1).close()
        helper.runMigrationsAndValidate(TEST_DB, 2, true, ZeroAIDatabase.MIGRATIONS[0]).close()
        helper.runMigrationsAndValidate(TEST_DB, 3, true, ZeroAIDatabase.MIGRATIONS[1]).close()
        val db =
            helper.runMigrationsAndValidate(
                TEST_DB,
                4,
                true,
                ZeroAIDatabase.MIGRATIONS[2],
            )
        val tables = queryTableNames(db)
    ...
    fun migrate_4_to_5_addsRemoteVersion() {
        ...
        helper.runMigrationsAndValidate(TEST_DB, 2, true, ZeroAIDatabase.MIGRATIONS[0]).close()
        helper.runMigrationsAndValidate(TEST_DB, 3, true, ZeroAIDatabase.MIGRATIONS[1]).close()
        helper.runMigrationsAndValidate(TEST_DB, 4, true, ZeroAIDatabase.MIGRATIONS[2]).close()
        val db =
            helper.runMigrationsAndValidate(
                TEST_DB,
                5,
                true,
                ZeroAIDatabase.MIGRATIONS[3],
            )
    ...
    fun migrate_5_to_6_addsImagesJson() {
        ...
        helper.runMigrationsAndValidate(TEST_DB, 3, true, ZeroAIDatabase.MIGRATIONS[1]).close()
        helper.runMigrationsAndValidate(TEST_DB, 4, true, ZeroAIDatabase.MIGRATIONS[2]).close()
        helper.runMigrationsAndValidate(TEST_DB, 5, true, ZeroAIDatabase.MIGRATIONS[3]).close()
        val db =
            helper.runMigrationsAndValidate(
                TEST_DB,
                6,
                true,
                ZeroAIDatabase.MIGRATIONS[4],
            )
    ...
    fun migrate_6_to_7_addsUniqueIndex() {
        ...
        helper.runMigrationsAndValidate(TEST_DB, 4, true, ZeroAIDatabase.MIGRATIONS[2]).close()
        helper.runMigrationsAndValidate(TEST_DB, 5, true, ZeroAIDatabase.MIGRATIONS[3]).close()
        helper.runMigrationsAndValidate(TEST_DB, 6, true, ZeroAIDatabase.MIGRATIONS[4]).close()
        val db =
            helper.runMigrationsAndValidate(
                TEST_DB,
                7,
                true,
                ZeroAIDatabase.MIGRATIONS[5],
            )
        val indexes = queryIndexNames(db)
        db.close()
    ...
    /**
     * Verifies migration 11 to 12 inserts the Twitter Browse official plugin row.
     */
    @Test
    fun migrate_11_to_12_insertsTwitterBrowseOfficialPlugin() {
        ...
        val db =
            helper.runMigrationsAndValidate(
                ...
                12,
                true,
                ZeroAIDatabase.MIGRATIONS.last(),
            )
        val pluginIds = queryPluginIds(db)
        db.close()

        assert(pluginIds.contains("official-twitter-browse")) {
            "Migration 11→12 did not insert official-twitter-browse"
        }
    }

    /**
     * Verifies a full migration from version 1 through version 12 succeeds.
     */
    @Test
    fun migrate_1_to_12_fullChain() {
        helper.createDatabase(TEST_DB, 1).close()
        val db =
            helper.runMigrationsAndValidate(
                TEST_DB,
        ...
        assert(tables.contains("terminal_entries")) { "Missing terminal_entries table" }
        assert(!tables.contains("chat_messages")) { "chat_messages table should have been removed" }
        assert(pluginIds.contains("official-twitter-browse")) {
            "Missing official-twitter-browse plugin after full migration"
        }
    }
    ...
    private fun queryPluginIds(db: androidx.sqlite.db.SupportSQLiteDatabase): Set<String> {
        val cursor = db.query("SELECT id FROM plugins")
