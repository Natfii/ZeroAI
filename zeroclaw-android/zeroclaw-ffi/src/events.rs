...
// TODO: Call this from `start_daemon_inner` (runtime.rs) after skills are
// loaded via `load_skills_with_config`. Currently the upstream `skills`
// module is `pub(crate)` so the FFI crate cannot access `Skill::tools()`
// directly. Once upstream exposes a public skill-tool listing API, wire
// it here.
#[allow(dead_code)]
