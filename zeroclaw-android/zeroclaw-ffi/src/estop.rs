...
pub(crate) fn is_engaged() -> bool {
    ESTOP_ENGAGED.load(Ordering::Acquire)
}
...
pub(crate) fn load_state(data_dir: &Path) {
    ...
    ESTOP_ENGAGED.store(state.kill_all, Ordering::Release);
    if state.kill_all {
...
pub(crate) fn engage_estop_inner() -> Result<(), FfiError> {
    ESTOP_ENGAGED.store(true, Ordering::Release);
...
pub(crate) fn get_estop_status_inner() -> Result<FfiEstopStatus, FfiError> {
    let engaged = ESTOP_ENGAGED.load(Ordering::Acquire);
...
pub(crate) fn resume_estop_inner() -> Result<(), FfiError> {
    ESTOP_ENGAGED.store(false, Ordering::Release);
