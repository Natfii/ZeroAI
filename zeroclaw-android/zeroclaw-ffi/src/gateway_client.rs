...
/// Shared HTTP client for all gateway calls.
///
/// Reusing a single client avoids per-request TCP connection setup and
/// connection pool allocation overhead.
static GATEWAY_CLIENT: OnceLock<reqwest::Client> = OnceLock::new();
...
/// Returns the shared gateway HTTP client, creating it on first access.
///
/// Uses `get()` + `set()` because `get_or_try_init` is still unstable.
/// A harmless data race is possible: two threads may both build a client,
/// but only one wins the `set()` and the other is dropped.
fn get_client() -> Result<&'static reqwest::Client, FfiError> {
    if let Some(client) = GATEWAY_CLIENT.get() {
        return Ok(client);
    }
    ...
    let _ = GATEWAY_CLIENT.set(client);
    GATEWAY_CLIENT.get().ok_or_else(|| FfiError::SpawnError {
        detail: "GATEWAY_CLIENT initialization race — client unavailable".into(),
    })
}
