...
/// Local-only routing config for the Nano Triage routing selector.
///
/// Maps a [`RouteHint`] to a preferred ordered list of provider names.
/// Defined locally because upstream zeroclaw has no equivalent.
#[derive(Debug, Default, Clone, serde::Serialize, serde::Deserialize)]
pub struct RoutingConfig {
    /// Providers preferred for `RouteHint::Simple` (quick lookups, factuals).
    #[serde(default)]
    pub simple: Vec<String>,
    /// Providers preferred for `RouteHint::Complex` (multi-step reasoning).
    #[serde(default)]
    pub complex: Vec<String>,
    /// Providers preferred for `RouteHint::Creative` (writing, ideation).
    #[serde(default)]
    pub creative: Vec<String>,
    /// Providers preferred for `RouteHint::ToolUse` (function-calling).
    #[serde(default)]
...
/// handles the request. The agent loop uses this to select the
