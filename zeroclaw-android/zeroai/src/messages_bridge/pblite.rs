...
/// Heuristic for distinguishing a base64-encoded `bytes` field from a
/// genuine UTF-8 string when the JSON-layer type system can't tell them
/// apart.
///
/// Returns `true` when the string contains characters that don't appear
/// in ordinary message text (`+`, `/`, `=`) or is long enough that the
/// base64 interpretation is likely intentional. This intentionally
/// errs on the side of treating ambiguous strings as UTF-8 — plain
/// ASCII like "test" must round-trip as a string.
fn looks_like_base64(s: &str) -> bool {
    s.contains('=') || s.contains('+') || s.contains('/') || s.len() >= 16
}
    ...
    match val {
        ...
        Value::String(s) => {
            ...
            if let Ok(n) = s.parse::<u64>() {
                ...
                && let Ok(decoded) = BASE64.decode(s.as_bytes())
            {
                // Looks like base64-encoded bytes (has padding `=` or
                // contains `+/` — characters that never appear in
                // ordinary message text). Plain ASCII strings like
                // "test" or "hello" would otherwise decode silently as
                // base64 and corrupt the wire format.
