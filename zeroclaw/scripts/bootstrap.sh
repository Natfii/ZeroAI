...
Options:
  ...
  --provider <id>            Provider for non-interactive onboarding (default: anthropic)
...
Examples:
  ...
  ./zeroclaw_install.sh --onboard --api-key "sk-..." --provider anthropic [--model "claude-sonnet-4-6"]
...
Environment:
  ...
  ZEROCLAW_PROVIDER          Used when --provider is not provided (default: anthropic)
...
install_prebuilt_binary() {
  ...
  # No remote release artifacts available for this private project.
  warn "Pre-built binary installation is not available."
  return 1
...
PROVIDER="${ZEROCLAW_PROVIDER:-anthropic}"
...
Next steps:
  ./zeroclaw_install.sh --docker --interactive-onboard
  ./zeroclaw_install.sh --docker --api-key "sk-..." --provider anthropic
DONE
  exit 0
fi
