...
impl AuthService {
    ...
    pub async fn store_openai_tokens(
        ...
        let mut profile = AuthProfile::new_oauth("openai", profile_name, token_set);
    ...
    pub async fn get_valid_openai_access_token(
        ...
        let Some(token_set) = profile.token_set.as_ref() else {
            anyhow::bail!("OpenAI auth profile is not OAuth-based: {profile_id}");
        };
        ...
        let Some(latest_tokens) = latest_profile.token_set.as_ref() else {
            anyhow::bail!("OpenAI auth profile is missing token set: {profile_id}");
        };
...
mod tests {
    ...
    fn select_profile_prefers_override_then_active_then_default() {
        ...
        let id_active = profile_id("openai", "work");
        ...
        data.active_profiles
            .insert("openai".into(), id_active.clone());

        assert_eq!(
            select_profile_id(&data, "openai", Some("default")),
            Some(id_default)
        );
        assert_eq!(select_profile_id(&data, "openai", None), Some(id_active));
    }
}
