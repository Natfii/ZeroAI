use crate::config::schema::{
    DmPairingMode, StreamMode, TelegramParseMode, TelegramReactionLevel, TelegramReceiveMode,
    default_history_limit, default_text_chunk_limit,
};
use crate::config::{
    AutonomyConfig, BrowserConfig, ChannelsConfig, ComposioConfig, Config, DiscordConfig,
    HeartbeatConfig, MemoryConfig, ObservabilityConfig, RuntimeConfig, SecretsConfig,
    StorageConfig, TelegramConfig, TunnelConfig, WebhookConfig,
};
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    print_step(4, 8, "Tool Mode & Security");
    let (composio_config, secrets_config) = setup_tool_mode()?;

    print_step(5, 8, "Hardware (Physical World)");
    ...
    let config = Config {
        ...
        shared_folder: crate::config::schema::SharedFolderConfig::default(),
        tailscale_peers: crate::config::schema::TailscalePeersConfig::default(),
        ...
        twitter_browse: crate::config::TwitterBrowseConfig::default(),
...
fn allows_unauthenticated_model_fetch(provider_name: &str) -> bool {
    matches!(
        ...
        "ollama" | "llamacpp" | "sglang" | "vllm" | "osaurus" | "astrai" | "nvidia"
    )
}
...
fn curated_models_for_provider(provider_name: &str) -> Vec<(String, String)> {
    match canonical_provider_name(provider_name) {
        ...
        "openai" => vec![
            (
                "gpt-5.4".to_string(),
                "GPT-5.4 (best general/coding default)".to_string(),
            ),
            (
                "gpt-5-chat-latest".to_string(),
                "GPT-5 Chat (matches the ChatGPT product surface)".to_string(),
            ),
            (
            ...
            (
                ...
                "GPT-5.3 Codex (dedicated agentic coding)".to_string(),
            ),
        ],
        "nvidia" => vec![
            (
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    let tiers = vec![
        "⭐ Recommended (Anthropic, OpenAI, Gemini)",
...
fn provider_supports_device_flow(provider_name: &str) -> bool {
    matches!(canonical_provider_name(provider_name), "gemini")
}
...
mod tests {
    ...
    async fn quick_setup_model_override_persists_to_config_toml() {
        ...
        assert!(config_raw.contains("default_provider = \"anthropic\""));
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        tokio::fs::write(&config_path, "default_provider = \"anthropic\"\n")
            .await
            .unwrap();
    ...
    #[test]
    fn canonical_provider_name_normalizes_aliases() {
    ...
    #[test]
    fn provider_supports_device_flow_gemini() {
