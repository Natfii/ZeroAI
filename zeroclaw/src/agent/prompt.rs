...
/// Injects hub-app awareness fragments when present.
pub struct ConnectedAppsSection;

impl PromptSection for ConnectedAppsSection {
    fn name(&self) -> &str {
        "connected_apps"
    }

    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        match ctx.hub_app_context {
            Some(text) if !text.is_empty() => {
                Ok(format!("## Connected Apps\n\n{text}\n"))
            }
            _ => Ok(String::new()),
        }
    }
...
mod tests {
    ...
    #[test]
    fn connected_apps_section_renders_when_present() {
        let ctx = PromptContext {
            workspace_dir: Path::new("/tmp"),
            model_name: "test",
            tools: &[],
        ...
        let output = section.build(&ctx).unwrap();
        assert!(output.contains("## Connected Apps"));
        assert!(output.contains("Twitter/X: Connected."));
        assert!(output.contains("Email: Connected as test@example.com."));
    }

    #[test]
    fn connected_apps_section_empty_when_absent() {
        let ctx = PromptContext {
            workspace_dir: Path::new("/tmp"),
            model_name: "test",
            tools: &[],
