...
pub async fn process_message(config: Config, message: &str) -> Result<String> {
    ...
    if config.shared_folder.enabled {
        tool_descs.push(("shared_folder_list", "List files and directories in the user's shared folder."));
        tool_descs.push(("shared_folder_read", "Read a file from the user's shared folder."));
        tool_descs.push(("shared_folder_write", "Write a file or create a directory in the user's shared folder."));
    }
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
...
/// Process a message with an optional route hint from on-device classification.
///
/// When `hint` is `Some`, looks up the preferred provider order from
/// `config.routing` and uses it as the primary + fallback chain. When
/// `None`, uses `config.default_provider` with `config.reliability.fallback_providers`.
pub async fn process_message_routed(
    mut config: Config,
    message: &str,
    hint: Option<crate::router::RouteHint>,
) -> Result<String> {
    if let Some(h) = hint {
        let preferred = h.preferred_providers(&config.routing);
        tracing::debug!(?h, preferred_order = ?preferred, "Applying classified routing");
        if !preferred.is_empty() {
            config.default_provider = Some(preferred[0].clone());
            config.reliability.fallback_providers = preferred[1..].to_vec();
        }
    }
    process_message(config, message).await
}
