...
/// A domain-to-credential mapping for transparent auth header injection.
///
/// When `http_request` fires against a URL whose host matches `domain`,
/// the `auth_header_value` is injected as the `Authorization` header
/// unless the LLM already provided one.
pub(crate) struct SkillCredentialEntry {
    /// Exact domain to match (no subdomain matching).
    pub domain: String,
    /// Full header value (e.g. `"Bearer sk-abc123"`).
...
pub struct HttpRequestTool {
    ...
    timeout_secs: u64,
    skill_credentials: Vec<SkillCredentialEntry>,
}
...
impl Tool for HttpRequestTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Auto-inject skill credential if domain matches and no Authorization present.
        if let Ok(host) = super::extract_host(&url) {
            if let Some(cred) = self.skill_credentials.iter().find(|c| c.domain == host) {
                if !request_headers
                    .iter()
                    .any(|(k, _)| k.eq_ignore_ascii_case("authorization"))
                {
                    request_headers
                        .push(("Authorization".into(), cred.auth_header_value.clone()));
                }
            }
        }
...
mod tests {
    ...
    fn validate_requires_allowlist() {
        ...
        let tool = HttpRequestTool::new(security, vec![], 1_000_000, 30, Vec::new());
        let err = tool
    ...
    async fn execute_blocks_readonly_mode() {
        ...
        let tool =
            HttpRequestTool::new(security, vec!["example.com".into()], 1_000_000, 30, Vec::new());
        let result = tool
    ...
    #[test]
    fn skill_credential_matches_exact_domain() {
        let creds = vec![SkillCredentialEntry {
            domain: "zero-fans.com".into(),
            auth_header_value: "Bearer sk-test".into(),
        }];
        assert!(creds
            .iter()
            .find(|c| c.domain == "zero-fans.com")
            .is_some());
        assert!(creds
            .iter()
            .find(|c| c.domain == "evil.zero-fans.com")
            .is_none());
    }

    #[test]
    fn inject_skips_when_authorization_present() {
        let mut headers: Vec<(String, String)> =
            vec![("Authorization".into(), "Bearer user-provided".into())];
        let cred = SkillCredentialEntry {
            domain: "example.com".into(),
            auth_header_value: "Bearer auto-injected".into(),
        };
        if !headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("authorization"))
        {
            headers.push(("Authorization".into(), cred.auth_header_value.clone()));
        }
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0].1, "Bearer user-provided");
    }
}
