...
impl GeminiAuth {
    ...
    /// Whether this credential must use the internal Gemini CLI Code Assist API.
    fn uses_internal_code_assist_api(&self) -> bool {
...
/// Internal API endpoint used by Gemini CLI OAuth users.
...
impl GeminiProvider {
    ...
    fn build_generate_content_url(model: &str, auth: &GeminiAuth) -> String {
        if auth.uses_internal_code_assist_api() {
    ...
    /// Resolve the GCP project ID for Gemini CLI OAuth by calling the
    /// loadCodeAssist endpoint. Caches the result for subsequent calls.
    ...
    ///
    /// For Gemini CLI OAuth, pass the resolved `oauth_token` and `project`.
    /// For managed auth-profiles OAuth, pass `oauth_token` only.
    /// For API key auth, both are `None`.
    fn build_generate_content_request(
        ...
        let builder = match auth {
            ...
            GeminiAuth::ManagedOAuth => req
                .json(request)
                .bearer_auth(oauth_token.unwrap_or_default()),
            _ => req.json(request),
        };
        super::apply_custom_headers(builder, &self.custom_headers)
    }
...
impl GeminiProvider {
    async fn send_generate_content(
        ...
        if !response.status().is_success() {
            ...
            } else if auth.uses_internal_code_assist_api()
...
mod tests {
    ...
    #[test]
    fn managed_oauth_url_uses_public_endpoint() {
        let auth = GeminiAuth::ManagedOAuth;
        let url = GeminiProvider::build_generate_content_url("gemini-2.0-flash", &auth);
        assert!(url.contains("generativelanguage.googleapis.com/v1beta"));
        assert!(url.contains("models/gemini-2.0-flash"));
    ...
    #[test]
    fn managed_oauth_request_uses_public_payload_with_bearer_auth() {
        let provider = test_provider(Some(GeminiAuth::ManagedOAuth));
        let auth = GeminiAuth::ManagedOAuth;
        let url = GeminiProvider::build_generate_content_url("gemini-2.0-flash", &auth);
        let body = GenerateContentRequest {
            contents: vec![Content {
