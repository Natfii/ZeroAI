...
impl OpenAiProvider {
    pub fn new(credential: Option<&str>) -> Self {
        Self::with_base_url_and_reasoning(None, credential, None)
    }
    ...
    pub fn with_base_url(base_url: Option<&str>, credential: Option<&str>) -> Self {
        Self::with_base_url_and_reasoning(base_url, credential, None)
    }

    /// Create a provider with an optional custom base URL and reasoning-effort override.
    pub fn with_base_url_and_reasoning(
        base_url: Option<&str>,
    ...
    fn auto_reasoning_effort(model: &str) -> Option<ReasoningEffort> {
        match Self::normalized_model_name(model) {
            "gpt-5.4" => Some(ReasoningEffort::None),
            "gpt-5.3-codex" => Some(ReasoningEffort::Medium),
            normalized if normalized.starts_with("gpt-5.4-") => Some(ReasoningEffort::None),
            normalized if normalized.starts_with("gpt-5.3-codex-") => Some(ReasoningEffort::Medium),
            _ => None,
        }
    }
    ...
    fn effective_reasoning_effort(&self, model: &str) -> Option<String> {
        self.reasoning_effort
            .or_else(|| Self::auto_reasoning_effort(model))
    ...
    async fn warmup(&self) -> anyhow::Result<()> {
        if let Some(credential) = self.credential.as_ref() {
            let req = self
                ...
                .header("Authorization", format!("Bearer {credential}"));
            super::apply_custom_headers(req, &self.custom_headers)
                .send()
                .await?
                .error_for_status()?;
        }
...
mod tests {
    ...
    #[test]
    fn gpt_5_4_auto_reasoning_effort_defaults_to_none() {
        let provider = OpenAiProvider::new(Some("openai-test-credential"));
        assert_eq!(
            provider.effective_reasoning_effort("gpt-5.4").as_deref(),
            Some("none")
        );
    }

    #[test]
    fn gpt_5_3_codex_auto_reasoning_effort_defaults_to_medium() {
        let provider = OpenAiProvider::new(Some("openai-test-credential"));
        assert_eq!(
            provider
                .effective_reasoning_effort("gpt-5.3-codex")
                .as_deref(),
            Some("medium")
        );
    }
    ...
    #[test]
    fn explicit_reasoning_effort_override_wins() {
        let provider = OpenAiProvider::with_base_url_and_reasoning(
            None,
    ...
    fn request_serializes_reasoning_effort_when_present() {
        ...
        let req = ChatRequest {
            ...
            temperature: 0.7,
            reasoning_effort: provider.effective_reasoning_effort("gpt-5.4"),
        };
        let json = serde_json::to_string(&req).unwrap();
    ...
    #[test]
    fn native_request_serializes_reasoning_effort_when_present() {
        let provider = OpenAiProvider::new(Some("openai-test-credential"));
        let req = NativeChatRequest {
            ...
            temperature: 0.7,
            reasoning_effort: provider.effective_reasoning_effort("gpt-5.3-codex"),
            tools: None,
            tool_choice: None,
        };
        let json = serde_json::to_string(&req).unwrap();
