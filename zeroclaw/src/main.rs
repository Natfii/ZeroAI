...
mod providers;
mod runtime;
mod clawboy_triggers;
mod ffi_credential_hook;
mod messages_bridge;
mod router;
mod scripting;
mod security;
...
enum Commands {
    ...
    Onboard {
        ...
        /// Provider name (used in quick mode, default: anthropic)
        #[arg(long)]
...
channels (Telegram, Discord, CLI), heartbeat monitor, and \
...
Examples:
  ...
  zeroclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    ...
    /// Manage channels (telegram, discord, cli)
    #[command(long_about = "\
Manage communication channels.

Add, remove, list, and health-check channels that connect ZeroClaw \
to messaging platforms. Supported channel types: telegram, discord, cli.
...
#[derive(Subcommand, Debug)]
enum AuthCommands {
    /// Login with OAuth (OpenAI or Gemini)
    Login {
    ...
    match auth_command {
        AuthCommands::Login {
            ...
            match provider.as_str() {
                ...
                "openai" => {
                    if device_code {
                        match auth::openai_oauth::start_device_code_flow(&client).await {
                            Ok(device) => {
                                ...
                                println!("Saved profile {profile}");
                                println!("Active profile for openai: {profile}");
                                return Ok(());
                            }
                            Err(e) => {
                                println!(
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `zeroclaw auth paste-redirect --provider openai --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                _ => {
                    bail!("`auth login` supports --provider openai or gemini, got: {provider}");
                }
            }
        }
            ...
            match provider.as_str() {
                "openai" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `zeroclaw auth login --provider openai` first."
                        )
                    })?;
                ...
                _ => {
                    bail!("`auth paste-redirect` supports --provider openai or gemini");
                }
            }
            Ok(())
        }
        ...
        AuthCommands::Refresh { provider, profile } => {
            ...
            match provider.as_str() {
                "openai" => {
                    ...
                    {
                        Some(_) => {
                            println!("OpenAI token is valid (refresh completed if needed).");
                            Ok(())
                        }
                        None => {
                            bail!(
                                "No OpenAI auth profile found. Run `zeroclaw auth login --provider openai`."
                            )
                        }
                    }
                }
                ...
                _ => bail!("`auth refresh` supports --provider openai or gemini"),
            }
        }
