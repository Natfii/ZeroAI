...
///
/// Returns `true` if:
/// 1. `brain.db` does NOT exist **or** contains zero rows
/// 2. `MEMORY_SNAPSHOT.md` DOES exist
///
/// Previous implementation used file-size heuristics (`< 4096` bytes),
/// which produced false positives under WAL mode: the main `.db` file
/// can stay small while actual data lives in the `-wal` journal.
/// Opening the database triggers WAL recovery, giving an accurate count.
pub fn should_hydrate(workspace_dir: &Path) -> bool {
    ...
    // Open the DB (triggers WAL recovery) and check for actual rows.
    match Connection::open(&db_path) {
        Ok(conn) => {
            ...
            let count: i64 = conn
                ...
                .unwrap_or(-1);
            // -1 means the table doesn't exist yet → treat as empty.
            count <= 0
        }
        Err(_) => true,
    }
...
"#;
    ...
    /// Regression test: deleting a core memory then re-exporting the
    /// snapshot must remove that entry from `MEMORY_SNAPSHOT.md`.
    /// A subsequent hydration must NOT resurrect the deleted entry.
    #[test]
    fn delete_then_export_removes_entry_from_snapshot() {
        let tmp = TempDir::new().unwrap();
        let workspace = tmp.path();
        ...
        conn.execute(
            "INSERT INTO memories (id, key, content, category, created_at, updated_at)
             VALUES ('id2', 'secret', 'This should be forgotten', 'core', ?1, ?2)",
            params![now, now],
        )
        .unwrap();

        // Initial export — both entries in snapshot.
        let exported = export_snapshot(workspace).unwrap();
        assert_eq!(exported, 2);
        let snap = fs::read_to_string(snapshot_path(workspace)).unwrap();
        assert!(snap.contains("identity"));
        assert!(snap.contains("secret"));

        // Delete 'secret' from the database.
        conn.execute("DELETE FROM memories WHERE key = 'secret'", [])
            .unwrap();

        // Re-export — snapshot should no longer contain 'secret'.
        let exported = export_snapshot(workspace).unwrap();
        assert_eq!(exported, 1);
        let snap = fs::read_to_string(snapshot_path(workspace)).unwrap();
        assert!(snap.contains("identity"));
        assert!(!snap.contains("secret"), "deleted entry must not appear in snapshot");

        // Simulate cold boot: delete DB, hydrate from snapshot.
        drop(conn);
        fs::remove_file(&db_path).unwrap();
        assert!(should_hydrate(workspace));

        let hydrated = hydrate_from_snapshot(workspace).unwrap();
        assert_eq!(hydrated, 1, "only the surviving entry should be hydrated");
        ...
        let exists: bool = conn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM memories WHERE key = 'secret')",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert!(!exists, "deleted memory must not be resurrected by hydration");
    }

    /// WAL-mode regression: `should_hydrate` must return `false` when
    /// the database has rows, even if the main `.db` file is small due
    /// to un-checkpointed WAL data.
    #[test]
    fn should_hydrate_false_with_wal_data() {
        let tmp = TempDir::new().unwrap();
        let workspace = tmp.path();
        ...
        .unwrap();
        // Close WITHOUT checkpointing — data may live only in the WAL file.
        drop(conn);

        assert!(
            !should_hydrate(workspace),
            "should_hydrate must be false when DB has rows (even under WAL)"
        );
    }
}
