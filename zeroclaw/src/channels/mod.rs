...
pub mod cli;
pub mod discord;
pub mod discord_backfill;
pub mod discord_buffer;
pub mod telegram;
pub mod traits;
#[cfg(test)]
...
pub use discord::{DiscordChannel, set_linked_dm_user as set_linked_discord_dm_user};
...
/// Tag names whose paired blocks represent internal model reasoning.
///
/// Mirrors the FFI layer's `THINKING_TAG_NAMES` (`session.rs`) plus
/// `thought` and `antthinking` which OpenClaw also strips.
/// Ref: OpenClaw `src/shared/text/reasoning-tags.ts`
const REASONING_TAG_NAMES: &[&str] = &[
    "think",
    "thinking",
    "thought",
    "reasoning",
...
/// Strip reasoning/thinking XML tags from outgoing messages.
///
/// Scans for matched `<tag>...</tag>` pairs (case-insensitive) listed
/// in [`REASONING_TAG_NAMES`] and removes them entirely.  Tags inside
/// fenced code blocks (``` regions) are preserved so that code examples
/// containing these strings are not corrupted.
///
/// Design mirrors OpenClaw's `stripReasoningTagsFromText()` in strict
/// mode with "both" trim (ref: `src/shared/text/reasoning-tags.ts`).
fn strip_reasoning_tags(message: &str) -> String {
    if message.is_empty() {
        return String::new();
    }

    // Quick check: if no angle bracket exists, nothing to strip.
    let lower = message.to_lowercase();
    let has_any = REASONING_TAG_NAMES
        .iter()
    ...
    for tag in REASONING_TAG_NAMES {
        loop {
            let current_lower = clean.to_lowercase();
            let open = format!("<{tag}>");
            let close = format!("</{tag}>");

            let Some(open_start) = current_lower.find(&open) else {
                break;
            };

            // Recompute code-fence regions each iteration because
            // `replace_range` shifts byte offsets in the mutated string.
            let code_regions = find_code_fence_regions(&clean);
            if is_inside_code_region(open_start, &code_regions) {
                break;
            }

            let content_start = open_start + open.len();
            let Some(rel_close) = current_lower[content_start..].find(&close) else {
                // Unclosed tag — preserve it (strict mode).
                break;
            };
...
/// Find byte ranges of fenced code blocks (``` ... ```).
fn find_code_fence_regions(text: &str) -> Vec<(usize, usize)> {
    ...
    while let Some(start) = text[search_from..].find("```") {
        ...
        if let Some(end) = text[after_open..].find("```") {
            let abs_end = after_open + end + 3;
            regions.push((abs_start, abs_end));
            search_from = abs_end;
        } else {
            regions.push((abs_start, text.len()));
            break;
        }
    }
...
/// Check whether a byte offset falls inside any code-fence region.
fn is_inside_code_region(offset: usize, regions: &[(usize, usize)]) -> bool {
    regions.iter().any(|&(start, end)| offset >= start && offset < end)
}

/// Strip `[Used tools: ...]` lines from outgoing text.
///
/// `extract_tool_context_summary()` generates these annotations for
/// conversation history context.  The LLM sometimes parrots them back
/// verbatim in subsequent responses.
/// Ref: OpenClaw's `stripDowngradedToolCallText()` strips similar
/// `[Tool Call: ...]` artifacts (`src/agents/pi-embedded-utils.ts`).
fn strip_used_tools_lines(message: &str) -> String {
    if message.is_empty() || !message.contains("[Used tools:") {
        return message.to_string();
    }
    ...
    let result: String = message
        ...
        .map(|line| {
            ...
            if trimmed.starts_with("[Used tools:") && trimmed.ends_with(']') {
                ""
            } else {
                line
            }
...
/// Strip model-internal special tokens like `<|endoftext|>` or `<｜end▁of▁sentence｜>`.
///
/// Some providers (GLM, DeepSeek) leak these delimiter tokens into
/// completions.  The regex matches `<` followed by ASCII pipe `|` or
/// full-width pipe `｜`, arbitrary content, a closing pipe, then `>`.
/// Ref: OpenClaw `stripModelSpecialTokens()` in `pi-embedded-utils.ts`.
fn strip_model_special_tokens(message: &str) -> String {
    if message.is_empty() {
        return String::new();
    }
    ...
    while let Some((i, ch)) = chars.next() {
        if ch == '<' {
            if let Some(&(_, next_ch)) = chars.peek() {
                if next_ch == '|' || next_ch == '\u{FF5C}' {
                    if let Some(end) = find_special_token_end(&message[i..]) {
                        let skip_to = i + end;
...
/// Find the end offset of a `<|...|>` or `<｜...｜>` token starting at `s[0]`.
/// Returns byte offset past the closing `>`, or `None` if not a valid token.
fn find_special_token_end(s: &str) -> Option<usize> {
    let mut iter = s.char_indices().skip(1); // skip '<'
    let (_, pipe_open) = iter.next()?;
    if pipe_open != '|' && pipe_open != '\u{FF5C}' {
        return None;
    }
    ...
    for (j, ch) in iter {
        if ch == '>' {
            let before = &s[..j];
            let last_before = before.chars().next_back()?;
            if last_before == '|' || last_before == '\u{FF5C}' {
                return Some(j + 1);
            }
        }
        if j > 60 {
...
/// Sanitize outbound text for channel delivery.
///
/// Applies all stripping layers in order:
/// 1. Reasoning/thinking tags (`<think>`, `<thinking>`, etc.)
/// 2. Tool-call XML tags (`<tool_call>`, `<invoke>`, etc.)
/// 3. `[Used tools: ...]` annotation lines
/// 4. Model special tokens (`<|...|>`, `<｜...｜>`)
///
/// This is the single entry point for channel submodules.  Mirrors
/// OpenClaw's multi-layer pipeline (see plan doc for full mapping).
fn sanitize_outbound_text(message: &str) -> String {
    if message.is_empty() {
        return String::new();
    }

    let step1 = strip_reasoning_tags(message);
    let step2 = strip_tool_call_tags(&step1);
    let step3 = strip_used_tools_lines(&step2);
    let step4 = strip_model_special_tokens(&step3);
...
fn resolved_default_provider(config: &Config) -> String {
    config.effective_provider().to_string()
}
    ...
    let response = match command {
        ...
        ChannelRuntimeCommand::SetProvider(raw_provider) => {
            match resolve_provider_alias(&raw_provider) {
                Some(provider_name) => match get_or_create_provider(ctx, &provider_name, None).await {
                    Ok(_) => {
...
fn sanitize_channel_response(response: &str, tools: &[Box<dyn Tool>]) -> String {
    let known_tool_names: HashSet<String> = tools
        ...
        .collect();
    let step1 = strip_reasoning_tags(response);
    let step2 = strip_used_tools_lines(&step1);
    strip_isolated_tool_json_artifacts(&step2, &known_tool_names)
}
...
async fn process_channel_message(
    ...
    // Check start/stop triggers before the message reaches the LLM.
    // Uses spawn_blocking because the trigger handler calls block_on()
    // internally and we're already inside the tokio async runtime.
    if crate::clawboy_triggers::is_game_trigger(&msg.content)
        || crate::clawboy_triggers::is_stop_trigger(&msg.content)
    {
        if let Some(handler) = crate::clawboy_triggers::trigger_handler() {
            let handler: crate::clawboy_triggers::TriggerHandler = handler.clone();
            let msg_content = msg.content.clone();
            let msg_channel = msg.channel.clone();
            let response = tokio::task::spawn_blocking(move || {
                handler(&msg_content, &msg_channel)
            })
            .await
            .ok()
            .flatten();
            ...
            if let Some(response_text) = response {
                tracing::info!(
                    ...
                    channel = %msg.channel,
                    sender = %msg.sender,
                    "trigger matched in channel message — short-circuiting"
                );
    ...
    // Apply route-hint classification when the user hasn't explicitly set
    // a provider override via `/models`. If routing config has preferences
    // for the classified tier, use the first preferred provider and pass
    // the remaining providers as cascade fallbacks.
    let routing_fallbacks: Option<Vec<String>>;
    let has_explicit_override = ctx
        .route_overrides
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .contains_key(&history_key);
    if !has_explicit_override {
        let hint = crate::router::classify(&msg.content);
        let preferred = hint.preferred_providers(&ctx.routing);
        if !preferred.is_empty() {
            let old_provider = route.provider.clone();
            route.provider = preferred[0].clone();
            routing_fallbacks = Some(preferred[1..].to_vec());
            tracing::info!(
                from = %old_provider,
                to = %preferred[0],
                ?hint,
    ...
    let active_provider = match get_or_create_provider(ctx.as_ref(), &route.provider, routing_fallbacks).await {
    ...
    let tools_restricted = traits::take_tools_restricted(&msg.id);
    let all_tool_names: Vec<String> = if tools_restricted {
        ctx.tools_registry.iter().map(|t| t.name().to_string()).collect()
    } else {
        Vec::new()
    };

    let effective_excluded: &[String] = if tools_restricted {
        &all_tool_names
...
pub fn build_system_prompt_with_mode(
    ...
    if let Some(ctx) = hub_app_context.filter(|s| !s.is_empty()) {
        prompt.push_str("## Connected Apps\n\n");
        prompt.push_str(ctx);
        prompt.push('\n');
    }
    ...
    if let Some(email) = email_config.filter(|c| c.enabled) {
        let _ = writeln!(prompt, "## Email\n");
        let _ = writeln!(prompt, "You have your own email address: **{}**", email.address);
        let _ = writeln!(
            prompt,
            "Use the `email_check`, `email_read`, `email_reply`, `email_compose`, \
             `email_search`, `email_delete`, and `email_empty_trash` tools to interact \
             with your inbox."
        );
        if !email.check_times.is_empty() {
            let _ = writeln!(
                prompt,
                "Scheduled inbox checks are configured at: {}.",
                email.check_times.join(", ")
            );
        }
        prompt.push('\n');
    }
    ...
    if let Some(ref tg) = config.channels_config.telegram {
        channels.push(ConfiguredChannel {
            ...
            channel: Arc::new(
                ...
                .with_ack_reactions(tg.ack_reactions)
    ...
    if let Some(ref dc) = config.channels_config.discord {
        channels.push(ConfiguredChannel {
            ...
            channel: Arc::new(
                ...
                .with_ack_reactions(dc.ack_reactions),
            ),
        });
    }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    let channels = collect_configured_channels(&config, None, None);
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    tool_descs.push((
        "read_messages",
        "Read text messages from Google Messages conversations the user has allowlisted. Takes 'contact' (name), optional 'since' (ISO-8601), optional 'limit'. Returns timestamped transcript.",
    ));
    ...
    let discord_archive: Option<Arc<crate::memory::discord_archive::DiscordArchive>> =
        if config.channels_config.discord.is_some() {
            // config.config_path = data_dir/config.toml, so .parent() = data_dir.
            // DiscordArchive::open(data_dir) creates data_dir/memory/discord_archive.db.
            let archive_dir = config.config_path.parent().unwrap_or(&config.workspace_dir);
            match crate::memory::discord_archive::DiscordArchive::open(archive_dir) {
                Ok(a) => {
                    tracing::info!("Discord archive opened for channel listener");
                    Some(Arc::new(a))
                }
                Err(e) => {
                    tracing::warn!("Discord archive unavailable for listener: {e}");
                    None
                }
            }
        } else {
    ...
    let channels: Vec<Arc<dyn Channel>> =
        collect_configured_channels(&config, discord_archive, Some(Arc::clone(&mem)))
            .into_iter()
            .map(|configured| configured.channel)
            .collect();

    if channels.is_empty() {
        tracing::warn!("No channels configured; start_channels exiting early");
        println!("No channels configured. Run `zeroclaw onboard` to set up channels.");
        return Ok(());
    }

    let channel_names: Vec<&str> = channels.iter().map(|c| c.name()).collect();
    tracing::info!(channels = ?channel_names, "Starting channel listeners");
...
mod tests {
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            routing: Arc::new(crate::config::RoutingConfig::default()),
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_handles_models_command_without_llm_call() {
        ...
        provider_cache_seed.insert("anthropic".to_string(), fallback_provider);
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                content: "/models anthropic".to_string(),
                channel: "telegram".to_string(),
                timestamp: 1,
                thread_ts: None,
            },
        ...
        assert!(sent[0].contains("Provider switched to `anthropic`"));
    ...
    async fn process_channel_message_uses_route_override_provider_and_model() {
        ...
        let mut provider_cache_seed: HashMap<String, Arc<dyn Provider>> = HashMap::new();
        provider_cache_seed.insert("test-provider".to_string(), Arc::clone(&default_provider));
        provider_cache_seed.insert("anthropic".to_string(), routed_provider);
    ...
    fn conversation_memory_key_uses_message_id() {
        ...
        assert_eq!(conversation_memory_key(&msg), "telegram_U123_msg_abc123");
    }
