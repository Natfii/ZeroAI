...
/// Kept for the 19 existing unit tests that exercise the inner function.
#[cfg(test)]
fn strip_tool_call_tags(message: &str) -> String {
    super::strip_tool_call_tags(message)
}

/// Delegate to the shared outbound text sanitizer in the parent module.
fn sanitize_outbound_text(message: &str) -> String {
    super::sanitize_outbound_text(message)
}

/// Delegate to the shared `strip_reasoning_tags` in the parent module.
#[cfg(test)]
fn strip_reasoning_tags(message: &str) -> String {
    super::strip_reasoning_tags(message)
}

/// Delegate to the shared `strip_used_tools_lines` in the parent module.
#[cfg(test)]
fn strip_used_tools_lines(message: &str) -> String {
    super::strip_used_tools_lines(message)
}

/// Delegate to the shared `strip_model_special_tokens` in the parent module.
#[cfg(test)]
fn strip_model_special_tokens(message: &str) -> String {
    super::strip_model_special_tokens(message)
}
...
Allowlist Telegram username (without '@') or numeric user ID.",
    ...
    async fn try_parse_voice_message(&self, update: &serde_json::Value) -> Option<ChannelMessage> {
        ...
        let _ = (message, file_id, config);
        tracing::debug!("Voice transcription not available (transcription module removed)");
        None
    }
...
impl Channel for TelegramChannel {
    ...
    async fn finalize_draft(
        ...
        let text = &sanitize_outbound_text(text);
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        let content = sanitize_outbound_text(&message.content);
...
mod tests {
    ...
    #[test]
    fn strip_reasoning_tags_removes_think_tag() {
        let input = "<think>I should search for this</think>\nHere are the results.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Here are the results.");
    }

    #[test]
    fn strip_reasoning_tags_removes_thinking_tag() {
    ...
    #[test]
    fn strip_reasoning_tags_removes_reasoning_tag() {
        let input = "<reasoning>Step 1: analyze\nStep 2: conclude</reasoning>Final answer.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Final answer.");
    }

    #[test]
    fn strip_reasoning_tags_removes_analysis_tag() {
        let input = "Intro <analysis>deep thought</analysis> conclusion";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Intro  conclusion");
    }

    #[test]
    fn strip_reasoning_tags_removes_reflection_tag() {
        let input = "<reflection>hmm</reflection>Got it.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Got it.");
    }

    #[test]
    fn strip_reasoning_tags_removes_inner_monologue_tag() {
        let input = "<inner_monologue>thinking out loud</inner_monologue>\nResponse here.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Response here.");
    }

    #[test]
    fn strip_reasoning_tags_case_insensitive() {
        let input = "<Think>capital think</Think>\nResult.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Result.");
    }

    #[test]
    fn strip_reasoning_tags_multiple_blocks() {
        let input = "<think>first</think>A<thinking>second</thinking>B";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "AB");
    }

    #[test]
    fn strip_reasoning_tags_preserves_code_fences() {
        let input = "Normal text\n```\n<think>inside code block</think>\n```\nAfter code.";
        let result = strip_reasoning_tags(input);
        assert_eq!(
            result,
            "Normal text\n```\n<think>inside code block</think>\n```\nAfter code."
    ...
    #[test]
    fn strip_reasoning_tags_no_tags() {
        let input = "Just a normal message with no tags.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Just a normal message with no tags.");
    }

    #[test]
    fn strip_reasoning_tags_empty_input() {
        assert_eq!(strip_reasoning_tags(""), "");
    }

    #[test]
    fn strip_reasoning_tags_unclosed_tag_preserved() {
        let input = "<think>unclosed thinking without end tag";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "<think>unclosed thinking without end tag");
    }

    #[test]
    fn strip_reasoning_tags_removes_antthinking() {
        let input = "<antThinking>Claude artifact</antThinking>Clean text.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Clean text.");
    }

    #[test]
    fn strip_reasoning_tags_removes_thought() {
        let input = "<thought>provider thought</thought>Answer.";
        let result = strip_reasoning_tags(input);
        assert_eq!(result, "Answer.");
    }

    #[test]
    fn strip_reasoning_tags_outside_before_inside_code_fence() {
        let input = "<think>strip me</think>\nKeep this.\n```\n<analysis>inside code</analysis>\n```\nAlso keep.";
        let result = strip_reasoning_tags(input);
        assert_eq!(
            result,
            "Keep this.\n```\n<analysis>inside code</analysis>\n```\nAlso keep."
    ...
    #[test]
    fn strip_used_tools_lines_removes_single_line() {
        let input = "[Used tools: twitter_browse]\nHere are the results.";
        let result = strip_used_tools_lines(input);
        assert_eq!(result, "Here are the results.");
    }

    #[test]
    fn strip_used_tools_lines_removes_multiple_tools() {
        let input = "[Used tools: shell, web_search, http_request]\nDone.";
        let result = strip_used_tools_lines(input);
        assert_eq!(result, "Done.");
    }

    #[test]
    fn strip_used_tools_lines_midtext() {
        let input = "Intro.\n[Used tools: shell]\nResult.";
        let result = strip_used_tools_lines(input);
    ...
    #[test]
    fn strip_used_tools_lines_no_match() {
        let input = "Normal message with no tool lines.";
        let result = strip_used_tools_lines(input);
        assert_eq!(result, "Normal message with no tool lines.");
    }

    #[test]
    fn strip_used_tools_lines_empty() {
        assert_eq!(strip_used_tools_lines(""), "");
    }

    #[test]
    fn strip_used_tools_lines_only_tool_line() {
    ...
    #[test]
    fn strip_model_special_tokens_removes_ascii_pipes() {
        let input = "Hello <|endoftext|> world";
        let result = strip_model_special_tokens(input);
        assert_eq!(result, "Hello  world");
    }

    #[test]
    fn strip_model_special_tokens_removes_fullwidth_pipes() {
        let input = "Hello <\u{FF5C}end\u{2581}of\u{2581}sentence\u{FF5C}> world";
        let result = strip_model_special_tokens(input);
        assert_eq!(result, "Hello  world");
    }

    #[test]
    fn strip_model_special_tokens_removes_multiple() {
    ...
    #[test]
    fn strip_model_special_tokens_no_match() {
        let input = "Normal text with <html> tags";
        let result = strip_model_special_tokens(input);
        assert_eq!(result, "Normal text with <html> tags");
    }

    #[test]
    fn strip_model_special_tokens_empty() {
        assert_eq!(strip_model_special_tokens(""), "");
    }

    #[test]
    fn sanitize_outbound_text_strips_think_and_tool_tags() {
        let input = "<think>Let me search</think>\nHere is the answer.\n<tool_call>{\"name\":\"shell\"}</tool_call>";
        let result = sanitize_outbound_text(input);
        assert_eq!(result, "Here is the answer.");
    }

    #[test]
    fn sanitize_outbound_text_strips_used_tools_line() {
        let input = "[Used tools: twitter_browse]\nResults here.";
        let result = sanitize_outbound_text(input);
        assert_eq!(result, "Results here.");
    }

    #[test]
    fn sanitize_outbound_text_strips_special_tokens() {
        let input = "<|im_start|>assistant\nHello world<|im_end|>";
        let result = sanitize_outbound_text(input);
        assert_eq!(result, "assistant\nHello world");
    }
    ...
    #[test]
    fn sanitize_outbound_text_combined_real_world() {
        let input = "<think>I'll search for tweets about the Iran War again.\n</think>\n[Used tools: twitter_browse]\nHere are the results.";
        let result = sanitize_outbound_text(input);
        assert_eq!(result, "Here are the results.");
    }

    #[test]
    fn sanitize_outbound_text_preserves_normal_text() {
        let input = "Here are the search results for your query about climate change.";
        let result = sanitize_outbound_text(input);
        assert_eq!(result, input);
    }

    #[test]
    fn sanitize_outbound_text_preserves_code_blocks() {
