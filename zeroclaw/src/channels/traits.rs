...
///
/// Guild-aware fields like [`tools_allowed`](Self::tools_allowed) control
/// per-message tool access for Discord guild contexts.
#[derive(Debug, Clone)]
...
/// Per-message tool access control, stored outside the struct to avoid
/// breaking every existing construction site. Looked up by message ID
/// at dispatch time.
static TOOL_RESTRICTED_MESSAGES: parking_lot::Mutex<Option<std::collections::HashSet<String>>> =
    parking_lot::Mutex::new(None);

/// Marks a message as tool-restricted (no tool calls allowed).
pub fn mark_tools_restricted(message_id: &str) {
    let mut guard = TOOL_RESTRICTED_MESSAGES.lock();
    guard
        .get_or_insert_with(std::collections::HashSet::new)
        .insert(message_id.to_string());
}

/// Checks and consumes the tool-restriction flag for a message.
pub fn take_tools_restricted(message_id: &str) -> bool {
    let mut guard = TOOL_RESTRICTED_MESSAGES.lock();
    guard.as_mut().map_or(false, |set| set.remove(message_id))
}
