...
fn audit_manifest_file(root: &Path, path: &Path, report: &mut SkillAuditReport) -> Result<()> {
    ...
    if let Some(scripts) = parsed.get("scripts").and_then(toml::Value::as_array) {
        for (idx, script) in scripts.iter().enumerate() {
            let path = script.get("path").and_then(toml::Value::as_str);
            let runtime = script.get("runtime").and_then(toml::Value::as_str);

            let Some(path) = path else {
                report
                    .findings
                    .push(format!("{rel}: scripts[{idx}] is missing a path field."));
                continue;
            };

            let stripped = strip_query_and_fragment(path);
            if looks_like_absolute_path(stripped) {
                report.findings.push(format!(
                    "{rel}: scripts[{idx}].path must stay relative to the skill root ({path})."
                ));
            }
            ...
            {
                report.findings.push(format!(
                    "{rel}: scripts[{idx}].path must not escape the skill root ({path})."
                ));
            }
            if !has_supported_skill_script_suffix(stripped) {
                report.findings.push(format!(
                    "{rel}: scripts[{idx}].path must use a supported script extension (.rhai, .wasm, .py)."
                ));
            }
            if let Some(runtime) = runtime {
                let runtime = runtime.trim().to_ascii_lowercase();
                if !matches!(runtime.as_str(), "rhai" | "wasm" | "wasm-component" | "python") {
                    report.findings.push(format!(
                        "{rel}: scripts[{idx}].runtime uses an unsupported runtime ({runtime})."
                    ));
                }
            }
        }
    ...
    if let Some(triggers) = parsed.get("triggers").and_then(toml::Value::as_array) {
        for (idx, trigger) in triggers.iter().enumerate() {
            let Some(kind) = trigger.get("kind").and_then(toml::Value::as_str) else {
                report
                    .findings
                    .push(format!("{rel}: triggers[{idx}] is missing a kind field."));
                continue;
            };
            let kind = kind.trim().to_ascii_lowercase();
            if !matches!(
                kind.as_str(),
                "manual" | "cron" | "channel_event" | "provider_event"
            ) {
                report.findings.push(format!(
                    "{rel}: triggers[{idx}].kind uses an unsupported trigger type ({kind})."
                ));
            }
        }
    }
...
fn has_supported_skill_script_suffix(target: &str) -> bool {
    let lowered = target.to_ascii_lowercase();
    lowered.ends_with(".rhai") || lowered.ends_with(".wasm") || lowered.ends_with(".py")
}
...
"#,
    ...
    #[test]
    fn audit_accepts_supported_skill_scripts_and_triggers() {
        let dir = tempfile::tempdir().unwrap();
        let skill_dir = dir.path().join("scripted");
        std::fs::create_dir_all(skill_dir.join("scripts")).unwrap();
        std::fs::write(skill_dir.join("scripts").join("triage.rhai"), "status()").unwrap();
        std::fs::write(
            skill_dir.join("SKILL.toml"),
            r#"
[skill]
...
"#,
    ...
    #[test]
    fn audit_rejects_skill_script_path_escape() {
        let dir = tempfile::tempdir().unwrap();
        let skill_dir = dir.path().join("scripted");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
...
"#,
        ...
        assert!(
            report
                .findings
                .iter()
                .any(|finding| finding.contains("must not escape the skill root")),
            "{:#?}",
            report.findings
        );
    }
