...
impl EstopManager {
    ...
    /// Resume from an emergency stop.
    ///
    /// On Android, resume authorization is handled by device biometric
    /// authentication in the Kotlin layer before this function is called.
    pub fn resume(&mut self, selector: ResumeSelector) -> Result<()> {
        match selector {
...
mod tests {
    ...
    fn estop_levels_compose_and_resume() {
        ...
        manager
            .resume(ResumeSelector::Domains(vec!["*.chase.com".into()]))
            .unwrap();
        assert!(manager.status().blocked_domains.is_empty());
        assert!(manager.status().network_kill);

        manager
            .resume(ResumeSelector::Tools(vec!["shell".into()]))
            .unwrap();
