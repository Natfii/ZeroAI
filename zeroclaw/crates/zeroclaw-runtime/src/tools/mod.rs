...
/// Factory function that returns Android-local (or other embedder-local)
/// tools to be appended to every per-agent tool registry built by
/// [`all_tools_with_runtime`].
///
/// Re-introduces the extension point that earlier upstream releases
/// exposed via `set_global_extra_tools_factory`. The Android FFI uses
/// this to inject `twitter_read_profile`, the SAF-backed shared-folder
/// shim, and the Google Messages bridge tools into channel-handler
/// agents — without it, those tools appear only in the Terminal session
/// path and silently disappear from inbound Telegram/Discord traffic.
///
/// Set at process init via [`set_extra_tools_factory`]. The closure is
/// invoked fresh per registry build (so each agent's tools are
/// independent Box instances, not shared Arcs).
pub type ExtraToolsFactory = Box<dyn Fn() -> Vec<Box<dyn Tool>> + Send + Sync>;

static EXTRA_TOOLS_FACTORY: std::sync::OnceLock<ExtraToolsFactory> = std::sync::OnceLock::new();

/// Install the process-wide [`ExtraToolsFactory`]. Idempotent — only the
/// first call wins, matching the standard `OnceLock` semantics. Returns
/// `Err(())` on a re-install so callers can branch on "was this the
/// first install or a no-op" without having to handle the dropped
/// factory closure.
pub fn set_extra_tools_factory(factory: ExtraToolsFactory) -> Result<(), ()> {
    EXTRA_TOOLS_FACTORY.set(factory).map_err(|_| ())
}
...
pub fn all_tools_with_runtime(
    ...
    // Embedder-local extras (Android: `twitter_read_profile`,
    // SAF-backed shared folder, Google Messages bridge, …) get injected
    // via the process-wide factory set by [`set_extra_tools_factory`].
    // Without this, those tools would only be visible to the Terminal
    // session registry and silently disappear from channel-handler
    // agents — that gap was breaking Twitter / shared-folder access on
    // inbound Telegram and Discord messages.
    let mut registry = boxed_registry_from_arcs(tool_arcs);
    if let Some(factory) = EXTRA_TOOLS_FACTORY.get() {
