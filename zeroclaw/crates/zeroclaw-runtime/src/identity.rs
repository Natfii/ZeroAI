...
/// Maximum length (in chars) for the free-text `origin_story` field
/// when rendered into the system prompt.
///
/// Empirical sizing: persona-signal is concentrated in the first one
/// or two sentences ("an agent built to help indie devs ship faster",
/// "a curious archivist with a knack for finding things"); paragraphs
/// of additional backstory rarely change small-model behaviour and
/// inflate every turn's prefill. 240 chars accommodates a couple of
/// sentences without truncating the salient identity claim.
const AIEOS_ORIGIN_STORY_MAX_CHARS: usize = 240;

/// Truncates [`text`] to at most [`max_chars`] Unicode characters,
/// appending an ellipsis when truncation occurred. Operates on char
...
pub fn aieos_to_system_prompt(identity: &AieosIdentity) -> String {
    ...
    if let Some(ref hist) = identity.history {
        ...
        if let Some(ref story) = hist.origin_story {
            // Cap origin_story at AIEOS_ORIGIN_STORY_MAX_CHARS to keep
            // a single user-authored backstory from dominating the
            // system prompt. The first sentence or two carries the
            // persona signal; longer paragraphs are flavor that small
            // models cannot internalise and cloud models do not need
            // shipped on every turn. User-designed role intent is
            // preserved (we keep the start of the story); narrative
            // bloat at the tail end is dropped.
            let truncated = truncate_with_ellipsis(story, AIEOS_ORIGIN_STORY_MAX_CHARS);
            let _ = writeln!(prompt, "**Origin Story:** {}", truncated);
        }
